/* BDL PWA UPDATE 2026-09-16C — Catch Up + Previous Answer live */
...
self.addEventListener("fetch",event=>{
 ...
 if(request.mode==="navigate"){
  event.respondWith(fetch(request,{cache:"no-store"}).then(async r=>{const type=r.headers.get("content-type")||"";if(!type.includes("text/html"))return r;let text=await r.text();if(!text.includes("quiz-schedule-1530.js"))text=text.replace("</body>",'<script src="quiz-schedule-1530.js?v=20260916c"></script></body>');return new Response(text,{status:r.status,statusText:r.statusText,headers:r.headers})}).catch(()=>fetch(request,{cache:"no-store"})));return;
 }
 const fresh=new URL(url.href);if(url.pathname.endsWith("/results-menu.js"))fresh.searchParams.set("v","20260916c");if(url.pathname.endsWith("/test-platform.js"))fresh.searchParams.set("v","20260916c");if(url.pathname.endsWith("/quiz-schedule-1530.js"))fresh.searchParams.set("v","20260916c");event.respondWith(fetch(fresh.href,{cache:"no-store"}).catch(()=>fetch(request,{cache:"no-store"})));
});
