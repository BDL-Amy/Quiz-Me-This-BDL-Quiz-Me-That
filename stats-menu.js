...
(function(){
  ...
  function titleTop20(data){
    ...
    if(!rows.length){
      ...
      body=`<div style="background:#fff;border:1px solid var(--stats-light);border-radius:14px;overflow:hidden;min-width:470px">
        <div style="display:grid;grid-template-columns:34px minmax(90px,1fr) 60px 60px 54px 58px;align-items:center;gap:4px;padding:10px 8px;background:var(--stats);color:#fff;font-size:10px;font-weight:bold;text-align:center">
      ...
      rows.forEach(row=>{
        ...
        body+=`<div style="display:grid;grid-template-columns:34px minmax(90px,1fr) 60px 60px 54px 58px;align-items:center;gap:4px;padding:12px 8px;border-top:1px solid #eee;${me?'background:var(--stats-light);':''}text-align:center">
